...
int trace_entry_open(struct pt_regs *ctx, const char __user *filename, int flags)
{
...
int trace_entry_openat(struct pt_regs *ctx, int dfd, const char __user *filename, int flags)
{
        struct val_t val = {};
        u64 id = bpf_get_current_pid_tgid();
        u32 pid = id >> 32; // PID is higher part
...
int trace_entry_openat2(struct pt_regs *ctx, int dfd, const char __user *filename, struct open_how *how)
{
        int flags = how->flags;
...
print(b.get_syscall_prefix().decode() )
...
print(b.ksymname(fnname_openat2) )
...
b.attach_kprobe(event=fnname_open, fn_name="trace_entry_open")
b.attach_kretprobe(event=fnname_open, fn_name="trace_return")
b.attach_kprobe(event=fnname_openat, fn_name="trace_entry_openat")
b.attach_kretprobe(event=fnname_openat, fn_name="trace_return")
b.attach_kprobe(event=fnname_openat2, fn_name="trace_entry_openat2")
