...
print("%-18s %-16s %-6s %-20s %-5s %-5s %s" % ("TIME(s)", "COMM", "PID", "MESSAGE","PNT","CNT","parent"))
...
def print_event(cpu, data, size):
    ...
    if(s.find("terminal")!=-1):
        print("%-18.9f %-16s %-6d %-20s %-5d %-5d %d" % (time_s, event.comm, event.pid,
         "Hello, perf_output!",map_one.__getitem__(ct.c_uint32(event.pid)).value,map_two.__getitem__(map_two_s(event.comm)).value,map_three.__getitem__(ct.c_uint32(event.pid)).value))
